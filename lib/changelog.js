...
async function createChangelog(semver) {
  ...
  // Keep a "pure" changes list to return
  const changes = [];

  // Generate the list of changes
  commits.forEach((commit) => {
    const { message, hash } = commit;
    const commitMessage = createCommitMessage({ hash, message });
    changes.push(commitMessage);
    contents.push(commitMessage);
  });
...
async function updateChangelog({ forceMajor }) {
  ...
  if (!lastChangelogCommit) return undefined;
